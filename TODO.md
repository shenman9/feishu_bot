...
- **状态**：已在 `feishu_bot.py` 中完整实现。通过 GetMessage API 获取合并转发子消息，利用 `upper_message_id` 构建消息树，递归格式化为带发送者、时间戳和缩进层级的对话文本，包裹 `<forwarded_messages>` 标签后传递给插件处理。支持嵌套转发（深度上限 10 层）、媒体占位提示、interactive 卡片降级提取。
