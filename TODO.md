...
### 9. 支持读取飞书转发的合并消息（多选对话转发）

- **涉及文件**：`hub_bot.py`（消息分发入口）、`plugins/claude_code/claude_code_plugin.py`（消息处理）
- **问题**：飞书用户可以多选聊天记录进行合并转发，转发后的消息类型为 `merge_forward` 而非普通 `text`。当前机器人不识别该消息类型，直接忽略，用户无法通过转发对话内容来与机器人交互。
- **目标**：识别并解析飞书合并转发消息类型，提取其中的文本内容（按对话顺序拼接），作为用户输入传递给对应插件处理。需调研飞书开放平台对 `merge_forward` 消息的具体字段结构。
