...
- **历史会话恢复**：历史会话自动记录到磁盘（`data/claude_code/feishu_sessions.json`），发送 `/sessions` 可查看并一键恢复任意历史会话，重启服务不丢失
- **流式输出**：Claude Code 的输出实时推送到飞书卡片，节流更新（0.5 秒/50 字符）
- **过程可视化**：工具调用日志（含调用结果状态）与 Claude 文字回复按实际执行顺序交错展示，与原生 Claude Code 终端输出顺序一致；连续大量工具调用时自动折叠（保留最近 15 条）；任务执行中实时显示等待状态（`💭 思考中...` 表示等待模型推理，`⏳ 正在处理...` 表示工具执行中）及已等待秒数
...
|------|------|
| `/new` | 重置当前会话（保留工作目录）|
| `/sessions` | 列出最近 20 条历史会话，点击按钮可恢复任意一条 |
| `/cancel` | 终止当前正在运行的任务 |
...
> 输入不存在的 `/` 指令时，机器人会回复"未知指令"提示并引导发送 `/help`，不会将其作为 prompt 发送给 Claude Code。
